...
const Gallery: FC<GalleryProps> = ({
  ...
  // lock page scroll while lightbox is open (prevents background scrolling)
  // IMPORTANT: depend on `isOpen` (open/close only), not `lbIndex` (would relock on every arrow click)
  useEffect(() => {
    if (!isOpen) return
    lockScroll()
    return () => unlockScroll()
  ...
  return (
    <section className={rootClassName}>
      ...
      <Portal selector="#modal-root">
        <div
          className={classNames(styles.lightbox, { [styles.open]: lbIndex !== null })}
          onClick={closeLb}
          role="dialog"
          aria-modal="true"
          aria-hidden={lbIndex === null}
        >
          {lbIndex !== null && (
            <>
              <button type="button" className={styles.lbCloseBar} onClick={closeLb} aria-label="Close">
